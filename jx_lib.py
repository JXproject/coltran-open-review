...
def make_confusion_matrix(
    ...
    if percent:
        group_percentages = ["{0:.1f}%".format(value) for value in cf.flatten()*100]
    else:
...
def progress_plot(
    ...
    xs = list(range(1, 1+len(h.history['acc'])))
    # Plot
    fig = plt.figure(figsize=figsize)
    plt.subplot(2, 1, 1)
    plt.plot(xs, h.history['acc'], label="training")
    plt.plot(xs, h.history['val_acc'], label="validation")
    plt.ylabel("Accuracy")
    plt.xlabel("epoch")
    plt.xticks(xs)
    plt.legend()
